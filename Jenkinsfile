...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Validate code in CI') {
			...
			println 'non unix'
                 rc = bat returnStatus: true, script: "C:\Program Files\Salesforce CLI\bin\sfdx" force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile \"${jwt_key_file}\" --instanceurl ${SFDC_HOST}"
            }
